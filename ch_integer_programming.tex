...
value. Section~\ref{sec:lp-assumptions} flagged \emph{divisibility} as one
of four key assumptions, and we saw in the cabin
...
decision variables, so the formulation discipline of
Section~\ref{sec:structured-formulation} (sets, parameters, variables,
model) applies without modification. Only the domain of the variables
changes.
...
volume constraints of the cargo-mix problem in
Section~\ref{sec:cargo-mix}; only the domain of $x_k$ has changed.

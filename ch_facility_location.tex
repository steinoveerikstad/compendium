...
The model above fixes the route and opens a single station. The same cost
structure extends to a fleet that shares a small number of stations: the
planner chooses which stations to open, subject to a budget $S^{\max}$ on
their number, and each vessel then takes its cheapest available charging
plan. Figure~\ref{fig:fl-charging} shows the outcome for a four-vessel
fleet, and the \emph{value of the network} that the budget traces out.
